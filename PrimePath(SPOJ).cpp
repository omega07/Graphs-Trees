...
bool differby1(int a,int b) {
    int cnt = 0;
    string _1 = to_string(a), _2 = to_string(b);
...
void bfs(int n) {
    ...
    while(!q.empty()) {
        ...
        for(int i : graph[node]) {
            ...
            if(distance_[i] == -1) distance_[i] = distance_[node] + 1;
            else distance_[i] = min(distance_[i],distance_[node] + 1);
        }
    }
}
...
signed main() 
{
    ...
    test {
        ...
        distance_[a] = 0;
        bfs(a);
        if(distance_[b] == -1) cout<<"Impossible\n";
        else cout<<distance_[b]<<"\n";
